...
class StockBot():
    ...
    def __init__(self, symbol_key, s, dbPool, ofyers_instance):
        ...
        self.choppyLow = 40  # Changed threshold
        self.buy = False
        self.dbPool = dbPool
        ...
        self.vwapInitialized = False
        self.choppinessValue = Decimal('0')
        self.vwapValue = Decimal('0')
                ...
                if candle_time != current_candle_time:
                    if current_candle_data is not None:
                        self.candles.append({
                            ...
                            'volume': current_candle_data['volume'],
                        })
                    ...
                    current_candle_data['volume'] += volume
    ...
    async def indicatorLoop(self):
        """
        Calculate VWAP and Choppiness indicators
        """
        epsilon = 1e-10
        ...
        while self.running:
            ...
            async with self.dataLock:
                ...
                # Need at least choppyPeriod + 1 total data points for choppiness
            ...
            # Set desired precision (e.g., 6 decimal places)
            getcontext().prec = 8
            ...
            if len(self.candles) > 0:
                ...
                for candle in self.candles:
                    ...
                    typical_price = Decimal(str(candle['close']))
                    total_volume_price += typical_price * volume
                    total_volume += volume

                if total_volume > 0:
                    vwap_val = total_volume_price / total_volume
                else:
                    vwap_val = Decimal('0')

                async with self.dataLock:
                    self.vwapValue = vwap_val.quantize(Decimal('0.0001'))  # 4 decimal precision

                    if not self.vwapArray or self.vwapValue != self.vwapArray[-1]:
                        self.vwapArray.append(self.vwapValue)
                        print(f"[{self.ticker}] V:", self.vwapArray)

                    self.vwapInitialized = True
    ...
    async def buyLogic(self):
        while self.running:
            ...
            async with self.dataLock:
                ...
                currentVwap = self.vwapValue
                currentPrice = self.current_ltp
                localBuy = self.buy
                choppyReady = self.choppyInitialized
                vwapReady = self.vwapInitialized

            # Step 2: Check buy conditions
            # Buy if: price > VWAP AND chop < 40 AND ([0,1] or [1,1])
            isOpportunity = (
                    not localBuy
                    and choppyReady
                    and vwapReady
                    and currentPrice > currentVwap
                    and currentChoppy < self.choppyLow
                    and currentChart in ([0, 1], [1, 1])
            )
            ...
            try:
                ...
                if response.get('s') == 'ok':
                    async with self.dataLock:
                        ...
                        print(
                            f"[{self.symbol_key}] BUY EXECUTED: Price={self.buyPrice}, VWAP={currentVwap}, Chop={currentChoppy}")
                else:
    ...
    async def sellLogic(self):
        while self.running:
            ...
            # Sell if: red brick (direction = 0) OR market close time
            should_sell = (
                    localBuy and (
                    currentChart == [0]
            ...
            try:
                ...
                if response.get('s') == 'ok':
                    async with self.dataLock:
                        ...
                        print(f"[{self.symbol_key}] SELL EXECUTED: {tradeData}")
